...
export default function ModalUser({ item }) {
  ...
  const onPress = async () => {
    try {
      ...
      const response = await UserService.disableUser(object);
      ...
      >
        <ModalContent>
          <ModalHeader className="flex flex-col gap-1">
            Disable {item.name} {item.surname}?
          </ModalHeader>
          <ModalBody>
            <p>
              By disabling it, log-in related to this cannot be carried out.
            </p>
          </ModalBody>
          <ModalFooter>
